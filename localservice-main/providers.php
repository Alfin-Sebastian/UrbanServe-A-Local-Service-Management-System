...
if (isset($_GET['fav']) && isset($_GET['type']) && isset($_GET['id']) && isset($_SESSION['user'])) {
    ...
    header('Location: providers.php?' . http_build_query(array_diff_key($_GET, ['fav' => '', 'type' => '', 'id' => ''])));
    exit;
}
...
$sql .= " ORDER BY avg_rating DESC, u.name ASC LIMIT $offset, $per_page";
        ...
        <div class="providers-list">
            <?php if (count($providers) > 0): ?>
                <?php foreach ($providers as $provider): 
                    $is_favorite = in_array($provider['provider_id'], $favorite_providers);
                ...
                ?>
                    <div class="provider-card">
                        ...
                        <?php if (isset($_SESSION['user']) && in_array($_SESSION['user']['role'], ['customer', 'provider', 'admin'])): ?>
                            <a href="providers.php?<?= http_build_query(array_merge($_GET, ['fav' => $fav_action, 'type' => 'provider', 'id' => $provider['provider_id']])) ?>" 
